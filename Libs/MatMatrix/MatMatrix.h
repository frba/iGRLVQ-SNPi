...
template<class Number=DefaultNumberType> class MatMatrix
{
	...
	public:
		...
		MatMatrix& concatCols(const MatVector<Number> &v);
                MatMatrix& removeCols(const MatVector<unsigned int> &v);
 ...
 * @param m - Um vetor de inteiros contendo os indices das colunas a serem removidas
 *
 * @author Flavia Araujo
 * @since  22-02-2016
 */
template<class Number> MatMatrix<Number>& MatMatrix<Number>::removeCols(const MatVector<unsigned int> &v)
{
	
    register unsigned int i,j,l,k,counter;
    int newnCols=ncols-v.size();
    bool remove;
