...
enum operationType { column, row , diag , overall};
...
template<class Number=DefaultNumberType> class MatVector
{
	...
	public:
		...
		inline const Number & operator[](const int i) const;
                ...
                inline Number& x(void) { Number x=0; return nrows>0 ? vect[0] : x; };
                inline Number& y(void) { Number x=0; return nrows>1 ? vect[1] : x; };
                inline Number& z(void) { Number x=0; return nrows>2 ? vect[2] : x; };
                inline Number& t(void) { Number x=0; return nrows>3 ? vect[3] : x; };
                inline const Number& x(void) const { return nrows>0 ? vect[0] : 0; };
                inline const Number& y(void) const { return nrows>1 ? vect[1] : 0; };
                inline const Number& z(void) const { return nrows>2 ? vect[2] : 0; };
                inline const Number& t(void) const { return nrows>3 ? vect[3] : 0; };
...
template<class Number> std::string MatVector<Number>::toCSV() const
{
	...
	if(vect!=NULL)
	{
		...
		for(i=0;i<nrows;i++)
		{
			sprintf(strbuff,"%.17f",(double)vect[i]);
			outputS += strbuff;
			outputS += "\t";
		}
	}
