...
class LVQDSNode : public DSNode {
public:

    typedef std::map<LVQDSNode*, TConnection*> TPNodeConnectionMap; //Para mapeamento local dos n�s e conex�es ligadas a this
    ...
    LVQDSNode(int idIn, const TVector &v) : DSNode(idIn, v), wins(0), act(0), cls(0) {
    };
...
class LARFDSLVQ : public SOM<LVQDSNode> {
public:
    uint maxNodeNumber;
    float minwd;
    float e_b;
...
//                node.ds[i] = epsilon_ds;
    ...
    LARFDSLVQ& updateConnections(TNode *node) {
    ...
    LARFDSLVQ& updateAllConnections() {
    ...
    LARFDSLVQ& updateMap(const TVector &w) {
    ...
    inline LARFDSLVQ& getWinners(const TVector &w, TNode* &winner1, TNode* &winner2) {
    ...
    template<class Number> LARFDSLVQ& outputCentersDs(MatMatrix<Number> &m) {
        using namespace std;
